# Technical Report — Day-Ahead Solar Power Forecasting

**Plant**: Utility-scale solar PV, Karnataka, India (≈14.1°N, 77.3°E, 680 m ASL)  
**Task**: Predict hourly AC power (kW) for July 1 – August 31, 2025  
...
Given day-ahead weather forecasts (cloud cover, temperature, wind speed, humidity) issued for a single utility-scale solar plant, predict the mean AC power output (in kW) for each clock hour of the two-month test period (July–August 2025).
...
|------|------|--------|-------|
| `data/train.csv` | 12,978 | Jan 2024 – Jun 2025 | Includes measured plant data |
| `data/test.csv`  | 1,488  | Jul–Aug 2025         | Weather forecasts only |

**Training columns (plant-observed, test unavailable):**
- `ac_power_kw` — target variable: mean AC output over the hour
- `measured_poa_wm2` — plane-of-array irradiance (sensor)
- `measured_module_temp_c` — module temperature (sensor)
- `measured_ambient_temp_c` — ambient temperature (sensor)
- `rainfall_mm` — hourly rainfall
- `status` — SCADA operational state (NORMAL / STOP / PARTIAL)

**Available in both train and test:**
- `timestamp` — hour beginning, Indian Standard Time (UTC+05:30)
- `forecast_cloud_cover` — day-ahead total cloud cover fraction (0–1)
- `forecast_temp_c` — day-ahead air temperature (°C)
- `forecast_wind_ms` — day-ahead wind speed (m/s)
- `forecast_humidity_pct` — day-ahead relative humidity (%)

> Since measured sensor columns do not exist in the test set, they are **never used as model features** — only the four forecast columns and physics-derived features.
...
Using `pd.to_datetime(..., format='mixed', dayfirst=True)` — the naive approach — silently swapped day and month on unambiguous ISO rows whenever both day and month were ≤ 12. This affected ~35% of all rows (confirmed: `2025-07-01` parsed as `2025-01-07`).
...
**Impact**: Clean training row count changed from 12,195 (buggy) → 12,304 (fixed). The date ordering of the training set materially changed — monsoon holdout masks, seasonal splits, and the OOF fold boundaries all shifted.
...
- `-999` / `-999.0` — missing/faulty sensor read
- `9999` / `9999.0` — overflow / instrument max
...
Frozen SCADA sensors produce runs of consecutive identical non-zero values. These appear as spurious constant plateaus in `ac_power_kw` (e.g., the same value repeated for 6–8 hours despite varying irradiance).

**Detection**: Any run of ≥¥ 4 consecutive identical non-zero values is flagged and replaced with `NaN`.
...
The training set contained **36 exact duplicate timestamp pairs (72 rows)**. These arise from logging system restarts or daylight-saving-time boundary artefacts. Duplicates are resolved by averaging numeric columns within each timestamp group — preserving the measurement information rather than discarding half.

---

### 3.5 SCADA State Filtering

Rows where `status ∈ {STOP, PARTIAL}` represent non-operational periods (grid trips, inverter faults, planned maintenance). These hours have near-zero or throttled output that bears no relationship to the weather. Training on them would teach the model to predict plant outages from weather inputs — an impossible task for a day-ahead forecast. They are removed.
...
Missing values in the four forecast columns (cloud cover, temperature, wind, humidity) are filled using linear interpolation, then forward-fill and backward-fill for edge cases. Because these are day-ahead forecasts issued once per day, gaps are typically short (1–2 hours) and linear interpolation is physically appropriate.
...
Each row's timestamp represents the **hour beginning** (e.g., `06:00`), but values represent the **mean over the full hour** (`06:00`–`07:00`). The representative instant is therefore the hour **midpoint** (`06:30`).
...
### 4.2 Solar Position (pvlib — Ineichen/Perez model)

Using the plant's exact coordinates (14.10°N, 77.28°E, 680 m, IST timezone) and `pvlib.location.Location.get_solarposition()`:

| Feature | Description |
|---------|-------------|
| `solar_elevation` | Sun's angle above horizon (degrees). Used for night masking |
| `solar_azimuth` | Compass bearing of the sun |
| `solar_zenith` | Complement of elevation (90° âˆ’ elevation) |
| `cos_zenith` | `max(0, cos(zenith))` — direct irradiance scaling factor |
| `is_day` | Binary: 1 if elevation > 0°, else 0 |
...
| Feature | Description |
|---------|-------------|
| `clearsky_ghi` | Global Horizontal Irradiance (W/m²) |
| `clearsky_dni` | Direct Normal Irradiance (W/m²) |
| `clearsky_dhi` | Diffuse Horizontal Irradiance (W/m²) |
| `clearsky_poa` | Plane-of-Array irradiance for panel tilt=15°, azimuth=180° South |

The POA calculation uses `pvlib.irradiance.get_total_irradiance()` with the plant's actual panel geometry.

---

### 4.4 Cloud Attenuation — Three Models
...
|---------|---------|----------------|
| `est_poa_linear` | `clearsky_poa × (1 âˆ’ cloud)` | Simple linear shading |
| `est_poa_quad`   | `clearsky_poa × (1 âˆ’ cloud)²` | Quadratic — penalises heavy cloud more |
| `est_poa_kasten` | `clearsky_poa × (1 âˆ’ 0.75 × cloudÂ³Â·)` | Kasten–Czeplak empirical cloud model |

The Kasten–Czeplak formula is the most physically accurate (calibrated against real sky measurements) and receives the most weight via the learned ensemble blending.
...
```
T_cell = T_air + POA / (Uâ‚€ + Uâ‚ × wind_speed)
```
...
```
P_dc = DC_capacity × (POA_kW) × (1 + Î± × (T_cell âˆ’ 25°C))
```
where `Î± = âˆ’0.0035 per °C` (âˆ’0.35%/°C, typical mono-Si panels).

**AC Power** (inverter conversion at 97% efficiency, capped at inverter rating):
```
P_ac = clip(P_dc × 0.97, 0, 10,000 kW)
```
...
| Feature | Formula |
|---------|---------|
| `sin_hour` / `cos_hour` | `sin(2Ï€ × hour / 24)`, `cos(2Ï€ × hour / 24)` |
| `sin_month` / `cos_month` | `sin(2Ï€ × month / 12)`, `cos(2Ï€ × month / 12)` |
| `sin_doy` / `cos_doy` | `sin(2Ï€ × day_of_year / 365.25)`, `cos(...)` |
| `solar_noon_diff` | `abs((hour + 0.5) âˆ’ 12.25)` — distance from solar noon |
...
**File**: `preprocessing/models.py` — `SolarForecastingEnsemble`
...
- **XGBoost** has stronger regularisation — more robust on small folds
...
| Row subsampling | 0.8 | 0.8 | — |
| Column subsampling | 0.8 | 0.8 | — |
| Early stopping | 50 rounds | 50 rounds | 50 rounds |

All models use early stopping against the held-out fold — actual tree counts vary by fold.
...
**File**: `train_and_predict.py` → calls `preprocessing/models.py`
...
After all 5 folds → 15 fitted models, full OOF predictions for every training row → learn NNLS meta-weights.
...
- **Seasonal irradiance patterns**: Even at the same cloud cover, July sun angles are different from January — the cyclical encodings carry this.
...
|------|-------|
| Total rows | 12,304 |
| Daytime rows (elevation > 0°) | 6,159 |
...
**File**: `train_and_predict.py` → Step 6–8

After training on the full dataset with the NNLS-learned weights:

1. **Apply the same feature pipeline** to `test.csv` — identical `clean_test_data()` + `add_physics_and_weather_features()` calls
...
Averaging across all 5 fold models (rather than retraining one final model) gives natural ensembling — each fold model saw a different 20% holdout during training, so their average is more robust than any single model trained on all data.

### 7.1 Test Period Characteristics

- **1,488 rows** (62 days × 24 hours) — validated row by row against original `test.csv`
- **683 night rows** → forced to 0.0 kW by post-processing
- **805 daytime rows** → GBDT ensemble predictions
- **Mean prediction (all hours)**: 1,236 kW
- **Peak prediction**: 9,278 kW (near but under 10,000 kW cap ✅)
...
```
Solar panels cannot generate negative power. GBDT models can produce small negative values at night due to noise — this clamps them.
...
```
Two conditions: (1) solar elevation ≤ 0° at hour midpoint (sun is below horizon), or (2) clear-sky POA ≤ 1 W/m² (astronomically negligible irradiance). Both evaluated at the hour midpoint to correctly handle sunrise/sunset hours.

**Verification**: 683 / 683 night rows = 0.0 kW ✅
...
All metrics are computed dynamically at runtime — no values are hardcoded anywhere.

### Baselines

**Persistence (tâˆ’24h)**: Use the same-hour reading from 24 hours prior. Common operational benchmark — if you can't beat persistence, the model adds no value.
...
| Metric | All-hours 5-Fold OOF | Monsoon Holdout (Jul–Aug 2024) |
|--------|---------------------|-------------------------------|
| **RMSE** | Primary — penalises large errors heavily (important for grid scheduling) | Tests generalisation to the target season |
| **MAE** | Secondary — robust to outliers, shows typical error |  |

### Monsoon Holdout Design

The holdout is **not** a random split — it's a **temporal seasonal split**: Jul 1 – Aug 31 2024 is withheld entirely, model trained on everything else, evaluated on this window. This is the most honest estimate of performance on the test period (which is also Jul–Aug), because it tests the model on a full monsoon season it has never seen.
...
The physics model assumes a fixed efficiency chain (STC DC capacity → inverter efficiency). In reality, module soiling, partial shading, inverter clipping behaviour, and SCADA response all introduce systematic biases the GBDT learns from historical patterns.

**Why does physics-only do *worse* than persistence on monsoon holdout?**
The theoretical model assumes forecast cloud cover is accurate. During monsoon, day-ahead forecasts are structurally unreliable — a 0.4 cloud cover forecast on a day that turns out fully overcast drives the physics model to predict 3,000 kW when actual is 200 kW. Persistence at least inherits yesterday's real measurement.

**Why is the ensemble's monsoon holdout similar to persistence?**
The ensemble also depends on `forecast_cloud_cover`. When that forecast is wrong, the ensemble is wrong too. The 2,400 kW monsoon RMSE is dominated by cloud forecast error — a fundamental limit of day-ahead solar forecasting during Indian monsoon with single-NWP inputs.

**The scatter plot interpretation**: Points clustering tightly along the perfect-fit diagonal across the full 0–10,000 kW range shows:
- No systematic bias at any power level
- R² = 0.9942 is genuine (the model is not just predicting the mean)
...
|-----------|------------|--------------------------------|
| Monsoon holdout RMSE 9× larger than CV RMSE | Only 1 prior monsoon season in training; cloud forecast error is high | Multi-year training data; probabilistic cloud ensemble (ECMWF ENS) |
